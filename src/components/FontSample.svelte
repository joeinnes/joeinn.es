<script>
  ...
  /** @type {Props} */
  let { fontName, fontSrc, wordTest = "Be Traist", pinHue } = $props();
  let lowercase = $state("");
  for (let i = 0; i < 26; i++) {
  ...
  const color =
    pinHue ||
    fontName.split("").reduce((acc, cur) => (acc += cur.charCodeAt(0)), 0) %
      360;
</script>
...
{#if fontName}
  ...
  >
    <div class="top">
      ...
      <p
        class="tracking-widest text-right"
        style="color: hsl({color}, 50%, 60%)"
      >
        {wordTest}
      </p>
    </div>
